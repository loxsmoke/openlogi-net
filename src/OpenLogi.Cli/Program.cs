...
switch (command)
{
    ...
    case "anim":
        ...
        break;
    case "pair":
        await PairAsync(args.Length > 1 && byte.TryParse(args[1], out var pt) ? pt : (byte)60);
        break;
    case "powerprobe":
        await PowerProbeAsync();
        break;
    case "powerset":
        await PowerSetAsync(args.Length > 1 ? Convert.ToByte(args[1], 16) : (byte)0);
        break;
    case "rawfeat":
        await RawFeatureAsync(Convert.ToUInt16(args[1], 16),
...
static async Task PairAsync(byte timeoutSec)
{
...
static async Task PowerProbeAsync()
{
    foreach (var inv in await Scan.Async())
        foreach (var dev in inv.Paired)
        {
...
static async Task PowerSetAsync(byte mode)
{
    foreach (var inv in await Scan.Async())
        foreach (var dev in inv.Paired)
        {
...
static async Task RawFeatureAsync(ushort featureId, byte function, byte[] fnArgs)
{
    foreach (var inv in await Scan.Async())
        foreach (var dev in inv.Paired)
        {
